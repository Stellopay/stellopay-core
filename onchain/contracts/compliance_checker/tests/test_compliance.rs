...
fn satisfied_rule(env: &Env) -> Rule {
    // CHECK: replace with the real constructor / real condition that is
    // trivially true against your test state fixture
    todo!()
}

/// Builds a rule that always evaluates as NOT satisfied.
fn unsatisfied_rule(env: &Env) -> Rule {
    // CHECK: replace with the real constructor / real condition that is
    // trivially false against your test state fixture
    todo!()
}

#[test]
fn and_group_rejects_when_only_one_of_two_rules_is_satisfied() {
    let env = Env::default();
    let checker = ComplianceChecker::new(&env); // CHECK: real setup

    let group = RuleGroup {
        logic: RuleLogic::And,
        rules: vec![satisfied_rule(&env), unsatisfied_rule(&env)],
    };

    let result = checker.check_transition(&env, &group /* , ...other real args */);
...
#[test]
fn or_group_allows_when_only_one_of_two_rules_is_satisfied() {
    let env = Env::default();
    let checker = ComplianceChecker::new(&env);
...
#[test]
fn nested_and_of_ors_evaluates_with_correct_precedence() {
    let env = Env::default();
    let checker = ComplianceChecker::new(&env);

    // Structure: AND( OR(satisfied, unsatisfied), OR(unsatisfied, unsatisfied) )
    // Outer AND requires BOTH inner OR groups to be true.
    // Inner OR #1 is true (one satisfied rule) — passes.
    // Inner OR #2 is false (no satisfied rules) — fails.
    // => overall result must be false (correct precedence: AND binds the
    //    two OR results, not a flat left-to-right scan of 4 leaf rules).
    let inner_or_true = RuleGroup {
        logic: RuleLogic::Or,
        rules: vec![satisfied_rule(&env), unsatisfied_rule(&env)],
    };
    let inner_or_false = RuleGroup {
        logic: RuleLogic::Or,
        rules: vec![unsatisfied_rule(&env), unsatisfied_rule(&env)],
    };

    let outer_and = RuleGroup {
        logic: RuleLogic::And,
        groups: vec![inner_or_true, inner_or_false], // CHECK: real field name for nested groups
        rules: vec![],
    };
    ...
    assert!(
        !result,
        "nested AND-of-ORs must reject when one inner OR group fails, \
         proving groups are evaluated with correct precedence rather than \
         a flat left-to-right scan of leaf rules"
    );
}

#[test]
fn nested_and_of_ors_allows_when_both_inner_or_groups_pass() {
    let env = Env::default();
    let checker = ComplianceChecker::new(&env);

    let inner_or_a = RuleGroup {
        logic: RuleLogic::Or,
        rules: vec![satisfied_rule(&env), unsatisfied_rule(&env)],
    };
    let inner_or_b = RuleGroup {
        logic: RuleLogic::Or,
        rules: vec![unsatisfied_rule(&env), satisfied_rule(&env)],
    };

    let outer_and = RuleGroup {
        logic: RuleLogic::And,
        groups: vec![inner_or_a, inner_or_b],
        rules: vec![],
    };
...
#[test]
fn and_group_short_circuits_without_evaluating_remaining_rules_after_first_failure() {
    // If the real implementation exposes a way to detect evaluation count
    // (e.g. a rule with a side-effecting counter, or an instrumented mock
    // rule), assert that a failing first rule in an AND group prevents
    // evaluation of subsequent rules. If no such hook exists in this
    // codebase, DELETE this test rather than fake it — don't assert
    // short-circuiting you can't actually observe.
}