...
//! - **Shared types** — `MilestoneView`, `MilestoneAgreementView`, `MilestoneAgreementStatus`
//!
//! ## Usage
//!
//! ```ignore
...
//! 2. **Changing a method signature** — parameter type, parameter order, or return type change
//!    breaks both callers and implementors at compile time.
//! 3. **Changing XDR-encoded type layouts** — adding, removing, or reordering fields on
//!    `#[contracttype]` structs or enums used as method parameters or return values breaks
//!    cross-contract calls at runtime even when Rust code compiles cleanly.
//! 4. **Narrowing a method's contract** — e.g. changing a previously documented "returns `None` on
//!    unknown id" guarantee to "panics on unknown id" is a semantic breaking change even if the
//!    signature is unchanged.
//! 5. **Changing the discriminant value of an existing enum variant** — XDR decoding on the calling
//!    side will misinterpret the value.
//!
...
//! major version bump:
//!
//! 1. **Adding a new method with a provided default body** — existing implementors continue to
//!    compile without change; the default is silently inherited.  Callers that depend on this crate
//!    gain access to the new method immediately.
//! 2. **Widening a method's contract** — e.g. changing a "may panic on unknown id" guarantee to
//!    "returns `None` on unknown id" is strictly more permissive and cannot break existing callers.
//! 3. **Adding a new associated type or constant** — provided the addition does not break existing
//!    `impl` blocks (i.e., the new item has a default or is not required to be implemented).
//! 4. **Adding a new variant to an enum** — if and only if (a) the new variant is appended at the
//!    end so existing discriminants are unchanged, and (b) call sites that match on the enum
//!    already handle a wildcard arm `_`.
//! 5. **Adding new `#[contracttype]` structs** that are not used as method parameters or return
//!    values yet — they are inert until referenced.
//! 6. **Improving or expanding doc comments** — purely documentary; no runtime impact.
//!
//! ## Implementor guidance
//!
//! Third-party contracts that implement `MilestoneContractInterface` should:
//!
//! 1. **Pin the `milestone-interface` version** in `Cargo.toml` using an exact or `~` specifier
//!    (e.g. `milestone-interface = { version = "=0.0.0", … }`). Open `*` ranges silently pick up
//!    breaking changes on re-build.
//! 2. **Add a conformance test** (see `test_milestone_interface_conformance` in
//!    `stello_pay_contract/tests/test_milestones.rs`) that exercises every `@stable` method via
//!    `MilestoneContractClient` and compares output against direct client calls.
//! 3. **Check `INTERFACE_VERSION`** at deploy time if your contract needs to guard against future
//!    interface changes (e.g. via a `static_assert`-style test that pins the expected version
//!    value).
//! 4. **Do not override `on_milestone_expired` with a panicking body** unless your contract can
//!    guarantee the hook is always called in a valid state. A panic inside the hook rolls back the
//!    entire `expire_milestone` transaction in the calling contract.
//!
//! ## Cross-reference
//!
...
//!   [`stello_pay_contract/tests/test_milestones.rs`](../../stello_pay_contract/tests/
//!   test_milestones.rs)
//! - Versioning and compatibility policy (extended prose): [`docs/state-machines.md § Milestone
//!   Interface Versioning`](../../../../../docs/state-machines.md)
...
/// 2. Keep the previous interface accessible under a versioned re-export or sibling crate
///    (`milestone-interface-v1`) for one full release cycle.
/// 3. Update `stello_pay_contract` to implement the new version.
/// 4. Update the conformance test in `stello_pay_contract/tests/test_milestones.rs` to pin the new
///    value.
...
///
/// - **No mutable state access** — query methods must be read-only.  Any implementation that writes
///   state in a query method violates the interface contract and may expose reentrancy attack
///   surfaces.
/// - **No panicking on valid input** — query methods must return `None` / `0` for unknown IDs
///   rather than panicking, so callers can distinguish "not found" from "error" without needing
///   `try_` variants.
/// - **Hook idempotency** — `on_milestone_expired` may be retried by the host on transient
///   failures.  Implementations should be idempotent or guard against double-invocation.
///
/// ## Cross-reference
///
...
pub trait MilestoneContractInterface {
    ...
    /// - The method name, parameter types, parameter order, and return type are frozen for the
    ///   lifetime of interface version 1.
    /// - The field set of `MilestoneView` (`id`, `amount`, `approved`, `claimed`) is frozen; new
    ///   fields may only be appended in a future major version.
    ...
    ///
    /// # Backward-compatibility guarantee
    /// - The method name, parameter types, and return type are frozen for the lifetime of interface
    ///   version 1.
    /// - The return value `0` for unknown agreements is a documented semantic guarantee, not an
    ///   implementation detail.  Changing it to a panic would be a breaking change.
    ...
    ///
    /// 1. Verified that the milestone is eligible for expiry (not already approved, claimed,
    ///    rejected, or previously expired).
    ...
    ///
    /// # Backward-compatibility guarantee
    /// - The method **signature** (`env`, `agreement_id`, `milestone_id`) is frozen for the
    ///   lifetime of interface version 1.
    /// - The **default body** (no-op) may be adjusted in minor versions as long as the observable
    ///   no-op contract (no state mutation, no events, no panics) is preserved.
